...
$result = Use-Invocation `
    -OperationText 'Initializing application database' `
    -OnSuccessText '[COMPLETED]' `
    -OnFailureText '[FAILED]' `
    -Invocation `
        { 
            Push-Location (Join-Path -Path $root -ChildPath 'src/JokesApi')

            & dotnet ef database update | Out-File -FilePath $database_log
...
if (-not $result)
{
    Write-Error "Cannot initialize application database. Please see '$database_log' for details."
    return
}

$result = Use-Invocation `
    -OperationText 'Starting self-host JokesApi services' `
    -OnSuccessText '[CONNECTED]' `
    -OnFailureText '[FAILED]' `
    -Invocation `
        { 
            $working = Join-Path -Path $root -ChildPath 'src/JokesApi/bin/x64/Debug/netcoreapp2.0'

            $env:ASPNETCORE_ENVIRONMENT = 'Development'
            $env:ASPNETCORE_URLS = 'http://*:59689'
            return Start-Process -FilePath 'dotnet' -WorkingDirectory $working -ArgumentList 'JokesApi.dll' -PassThru  -WindowStyle Minimized -ErrorAction Stop
        } `
    -Output $powershell_log `
    -PassThru
...
if ($result -eq $null)
{
    Write-Error "Cannot start self-host JokesApi service. Please see '$powershell_log' for details."
    return
}

$result = Use-Invocation `
    -OperationText 'Starting self-host JokesWeb services' `
    -OnSuccessText '[CONNECTED]' `
    -OnFailureText '[FAILED]' `
    -Invocation `
        { 
            $working = Join-Path -Path $root -ChildPath 'src/JokesWeb/bin/x64/Debug/netcoreapp2.0'

            $env:ASPNETCORE_ENVIRONMENT = 'Development'
            $env:ASPNETCORE_URLS = 'http://*:54036'
            return Start-Process -FilePath 'dotnet' -WorkingDirectory $working -ArgumentList 'JokesWeb.dll' -PassThru -WindowStyle Minimized -ErrorAction Stop
        } `
    -Output $powershell_log `
    -PassThru
...
if ($result -eq $null)
{
    Write-Error "Cannot start self-host JokesWeb service. Please see '$powershell_log' for details."
    return
}
